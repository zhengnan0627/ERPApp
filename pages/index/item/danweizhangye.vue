<template>
	<view class="container">
		<uni-nav-bar  fixed="true" left-icon="back"  @clickLeft="back"  title="单位账页查询"/>
		<view class="" style="position: fixed; top: 44px; left: 0; right: 0;">
			<view class="height-box" v-if="danweiinfo.dw_name">
				<view class="danjuinfo-box">
					{{danweiinfo.dw_name}}的账页明细
				</view>
			</view>		
		</view>
			...
			<view class="detail-box" :key="index">
				<view class="detail-box-item detail-box-flex">
					<view class="detailbox-flex-item">
						类型:<text style="margin-left: 2px;":style="{color:item.z_type == '借方'?'#ff0000':'#5555ff'}">{{item.z_type}}</text>
					</view>
					<view class="detailbox-flex-item">
						金额:<text style="margin-left: 2px; color: #000000;">{{item.amount}}</text>
					</view>
					<view class="detailbox-flex-item" style="flex: 1.2;">
						余额:<text style="margin-left: 2px; color: #000000;">{{item.left_amout}}</text>
					</view>
				</view>
				<view class="detail-box-item detail-box-flex">
					<view class="detailbox-flex-item" style="flex: 2;">
						单据编号:<text style="margin-left: 2px; color: #000000;">{{item.dj_bh}}</text>
					</view>
					<view class="detailbox-flex-item"style="flex: 1.2;">
						开票员:<text style="margin-left: 2px; color: #000000;">{{item.kpy_name}}</text>
					</view>
				</view>
				<view class="detail-box-item detail-box-flex">
					<view class="detailbox-flex-item"  style="flex: 2;">
					...
					<view class="detailbox-flex-item"style="flex: 1.2;">
						业务员:<text style="margin-left: 2px; color: #000000;">{{item.ywy_name}}</text>
					</view>
		...
		<u-loadmore :status="status" v-if="zyList.length > 5"/>	
	</view>
</template>
